...
Once every record has been [[identified|Identifying and splitting records]] using the non_tabular_row section of the mapping, data items can then be captured from each record.
...
h3. Capturing data from one line using line numbers:
...
+Note:+ [[Standard mappings|Standard YAML mappings]] are available on those fields with the functionality, eg:
...
h3. Capturing data from a range of lines using line numbers:
...
h3. Capturing data from a range of lines using Regular Expressions:
